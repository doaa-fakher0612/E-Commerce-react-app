...
export default function CartMenu({ cart, setCart }) {
 ...
 const updatedCart = [...cart]; // copy the original cart
  ...
  return (
    ...
    >
      {!cart.length ? <h4 className="text-center">There is No Items</h4> : (

            cart.map((product) => (
        <div id="cart-items" className="flex flex-col gap-2 w-full">
          <div id="poduct-in-cart" className="flex flex-row gap-4 w-full p-2">
            <div
              id="img-div"
              className="w-32 h-32 md:w-20 md:h-20 rounded flex flex-col gap-2"
            >
              <img
            ...
            <div id="data" className="flex flex-col justify-center gap-2">
              <h4>{product.title}</h4>
          ...
          <button className="rounded w-full p-3 bg-orange-500 text-white text-sm hover:bg-orange-600" onClick={()=>handelRemoveItem(product.id)}>Remove Item</button>
        </div>
      ))
    )
     }
